        ...
        if 'jam dumping' in df_filtered.columns:
            ...
            try:
                ...
                with st.form("filter_form_jam_dumping"):
                    ...
                    truck_options = ['Semua Dump Truck'] + df_filtered['dump truck'].unique().tolist() if 'dump truck' in df_filtered.columns else ['Semua Dump Truck']
                    ...
                    kategori_jam_options = (
                        ['Semua Kategori'] + 
                        sorted(df_filtered['kategori jam'].unique().tolist()) if 'kategori jam' in df_filtered.columns 
                        else ['Semua Kategori']
                    )
                    selected_kategori_jam = st.selectbox("Pilih Kategori Jam", kategori_jam_options)
                ...
                if refresh_button:
                    ...
                    # Hanya operator dari mitra yang dipilih
                    all_operators = set(df_filtered_period['nama operator'].unique())
                    ...
                    if selected_kategori_jam != "Semua Kategori" and 'kategori jam' in df_filtered.columns:
                        df_filtered_period = df_filtered_period[df_filtered_period['kategori jam'] == selected_kategori_jam]
                        # Cari operator yang tidak ada ritase pada kategori jam tersebut
                    ...
                    # Operator dengan ritase sesuai filter
                    operators_with_ritase = set(df_filtered_period['nama operator'].unique())
                    operators_with_ritases = list(
                        df_filtered_period['nama operator']
                        .dropna()  # Hilangkan nilai NaN
                        .loc[~df_filtered_period['nama operator'].str.contains("unknown", case=False, na=False)]  # Hilangkan "unknown"
                        .unique()
                    )

                    # Operator tanpa ritase dalam filter
                    operators_without_ritase = list(all_operators - operators_with_ritase)

                    # Buat DataFrame untuk operator tanpa ritase dengan kolom SPPH
                    df_operators_without_ritase = pd.DataFrame({
                        'nama_operator': operators_without_ritase,
                        'spph': [", ".join(selected_mitra)] * len(operators_without_ritase)  # Menambahkan informasi mitra yang dipilih
                    })
                    
                    # Buat DataFrame untuk operator dengan ritase dengan kolom SPPH
                    df_operators_with_ritases = pd.DataFrame({
                        'nama_operator': operators_with_ritases,
                        'spph': [", ".join(selected_mitra)] * len(operators_with_ritases)  # Menambahkan informasi mitra yang dipilih
                    })

                    # Atur index mulai dari 1
                    df_operators_without_ritase.index = range(1, len(df_operators_without_ritase) + 1)
                    df_operators_with_ritases.index = range(1, len(df_operators_with_ritases) + 1)

                    # Tampilkan hasil di Streamlit
                    st.subheader(f"Operator Tanpa Ritase Berdasarkan Kategori Jam")
                    st.write("**Filter Aktif:**")
                    st.write(f"- **Mitra/SPPH**: {', '.join(selected_mitra)}")
                    st.write(f"- **Dump Truck**: {selected_truck}")
                    st.write(f"- **Lokasi**: {selected_lokasi}")
                    st.write(f"- **Periode**: {selected_date_range[0]} s/d {selected_date_range[1]}")
                    st.write(f"- **Kategori Jam**: {selected_kategori_jam}")

                    if df_operators_without_ritase.empty:
                        st.info("Semua operator memiliki ritase berdasarkan filter yang dipilih.")
                    else:
                        st.write("Operator tanpa ritase pada jam yang dipilih")
                        st.dataframe(df_operators_without_ritase)
                        st.write("Operator dengan ritase pada jam yang dipilih")
                        st.dataframe(df_operators_with_ritases)

                        # Tambahkan tombol unduh untuk DataFrame operator tanpa ritase
                        csv_operators_without_ritase = df_operators_without_ritase.to_csv(index=False).encode('utf-8')
                        st.download_button(
                            label=f"Download Data Operator Tanpa Ritase",
                            data=csv_operators_without_ritase,
                            file_name=f'operator_tanpa_ritase_filtered.csv',
                            mime='text/csv'
                        )
                        
                        csv_operators_with_ritases = df_operators_with_ritases.to_csv(index=False).encode('utf-8')
                        st.download_button(
                            label=f"Download Data Operator Dengan Ritase",
                            data=csv_operators_with_ritases,
                            file_name=f'operator_dengan_ritase_filtered.csv',
                            mime='text/csv'
                        )
